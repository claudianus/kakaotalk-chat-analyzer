...
export function buildRuleBasedLlmFallback(data, plan, args) {
    const llmInsights = {};
    const archetype = fallbackRoomArchetype(data);
    if (archetype)
        llmInsights.roomArchetype = archetype;
    const bullets = data.highlights
        .map((h) => h.trim())
        .filter((h) => h.length > 4)
        .slice(0, 4);
    if (bullets.length)
        llmInsights.insightBullets = bullets;
...
//# sourceMappingURL=llm-rule-fallback.js.map