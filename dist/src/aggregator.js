...
function isDisplayKeywordToken(token) {
    const label = normalizeToken(token);
    if (label.length < 2)
        return false;
    if (KEYWORD_EXCLUDE.has(label) || DISPLAY_KEYWORD_STOPWORDS.has(label))
        return false;
    return !isNoiseKeyword(label);
}
function displayKeywordTokens(tokens) {
...
function dateKeyToUtcMs(dateStr) {
...
function trendItemsFromBuckets(buckets, topicLimit = 6) {
...
function buildDailyTopicTrend(dailyKeywordBuckets) {
    return trendItemsFromBuckets(dailyKeywordBuckets, 6);
}
function buildWeeklyTopicTrend(dailyKeywordBuckets) {
    ...
    for (const [dateStr, bucket] of dailyKeywordBuckets) {
        ...
        for (const item of bucket.topCounts(20))
            wb.addHits(item.label, item.count);
    }
    return trendItemsFromBuckets(weekBuckets, 6);
}
function buildMonthlyTopicTrend(dailyKeywordBuckets) {
...
export class ReportAggregator {
    ...
    firstDate = null;
    lastDate = null;
    replyLatencyByResponder = new Map();
    pendingQuestions = [];
    ...
    applyKeywordTokens(kwTokens, monthKey, dayKey) {
    ...
    consume(record, opts) {
        ...
        if (!isPureSystem) {
            ...
            if (this.prevSender !== null && record.sender !== this.prevSender && !isQuestion) {
                ...
                const idx = this.pendingQuestions.findIndex((q) => !q.answered && q.asker !== record.sender && q.time >= cutoff);
                if (idx >= 0) {
            ...
            if (isQuestion) {
                ...
                if (this.pendingQuestions.length > 50)
    ...
    finalize(meta, finalizeOpts) {
        ...
        const participantRoles = buildParticipantRoles(allParticipants, this.laughBySender, this.shortBySender, aliases);
        ...
        if (roomTotal > 0) {
            ...
            if (roomHonorificRatio >= 0.7)
                roomStyle = "formal";
            else if (roomCasualRatio >= 0.7)
                roomStyle = "casual";
        }
        const honorificInsight = participantHonorifics.length > 0
    ...
    buildQuestionAnswerTopology(aliases) {
        ...
        const allLatencies = [];
        for (const pair of this.questionAnswerPairs.values())
            allLatencies.push(...pair.latencies);
    ...
    buildBurstAnatomy(burstDays, avgDailyMessages, aliases) {
        return burstDays.slice(0, 5).map((d) => {
            const senderMap = this.dailySenderCounts.get(d.date) ?? new Map();
