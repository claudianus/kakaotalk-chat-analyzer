...
export function formatReplyGapMinutes(minutes) {
...
const MASKED_DISPLAY_NAME_RE = /(?<![\p{L}\p{N}])(?:[\p{L}\p{N}]\*{1,6}[\p{L}\p{N}](?![\p{L}\p{N}])|[\p{L}\p{N}]\*(?![\p{L}\p{N}*]))/gu;
function normalizeMarkdownAsterisks(text) {
    return text.replace(/[\uFF0A\u2217\u2731]/g, "*");
}
function protectMaskedDisplayNames(text) {
    const tokens = [];
...
function restoreMaskedDisplayNames(html, tokens) {
    let out = html;
