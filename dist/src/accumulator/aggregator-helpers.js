...
export function topCounts(map, limit) {
    return [...map.entries()]
...
/* ── 참여자·레이블 ── */
export function buildSenderLabels(senders, privacy) {
...
export function getDomains(message) {
...
/* ── 통계·지표 ── */
export function longestDateStreak(sortedYmd) {
...
export function computeGini(counts) {
    if (counts.length === 0)
        return null;
...
export function maxSilenceGapDays(sortedYmd) {
...
export function getParticipantStat(stats, sender) {
...
export function computeTop3Share(stats, total) {
    if (total === 0)
        return 0;
...
/* ── 시간대 ── */
export function computeDaypartPercents(hourly, total) {
    const bands = [
...
/* ── 키워드 ── */
export function top1ShareFromCounts(keywords, totalMessages) {
...
export function typeRichnessFromKeywords(keywords, totalMessages) {
...
export function splitMonthlyKeywordBuckets(buckets) {
...
export function topDailyLinkSpikes(dailyLinks) {
...
export function buildRoomEventStats(total, c, shopExtra) {
...
/* ── 관계 추론 ── */
export function inferRoomRelationship(honorific) {
...
//# sourceMappingURL=aggregator-helpers.js.map