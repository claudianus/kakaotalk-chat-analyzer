...
import { escapeHtml, formatNumber, formatReplyGapMinutes } from "./report-util.js";
...
function renderParticipantsFold(data) {
    ...
    if (n === 0)
        return "";
    return panel(`참여자 랭킹 · 전체 ${formatNumber(n)}`, "메시지 수 기준 참여자 랭킹.", renderParticipants(data.participants));
}
...
function renderSampleBadge(data) {
    const s = data.summary;
...
function giniMetricSub(gini, participants) {
    if (gini === null)
        return `표본 n=${participants}명 — 지니 산출 불가`;
    const band = gini < 0.42 ? "고른 참여(상대)" : gini < 0.62 ? "중간 쏠림" : "소수 집중(상대)";
    return `${band} · n=${participants}`;
}
function top3MetricSub(top3, gini) {
    const skew = top3 >= 70 ? "상위 3명이 대부분" : top3 >= 50 ? "상위가 절반 이상" : "비중 분산";
...
function renderParticipants(participants) {
    ...
    const rankBadge = (i) => {
        if (i === 0)
            return '<span class="rank-badge rank-gold">🥇</span>';
        if (i === 1)
            return '<span class="rank-badge rank-silver">🥈</span>';
        if (i === 2)
            return '<span class="rank-badge rank-bronze">🥉</span>';
...
function renderToneSignalsPanel(data) {
    const blocks = [];
    const hasExpressionRisk = data.profanity.messagesWithProfanity > 0 ||
    ...
    if (blocks.length === 0) {
        return panel("톤·감정 신호", "비속어·감정 분석 요약.", `<p style="margin:0;color:var(--muted);font-size:13px">감지된 비속 패턴이 없거나, 감정 분석 샘플이 부족합니다.</p>`);
    }
...
function renderSentimentInline(sentiment, hasExpressionRisk = false) {
    const compoundLabel = sentiment.compoundScore > 15 ? "전반적으로 긍정적" : sentiment.compoundScore < -15 ? "전반적으로 부정적" : "명시 감정어는 중립에 가까움";
...
function renderReactionsPanel(data) {
    ...
    if (data.pureLaughMessages > 0) {
        ...
        parts.push(`<div class="reaction-card" data-observe><span class="reaction-emoji">😂</span><div class="reaction-card-body"><span class="reaction-label">ㅋㅋ·ㅎㅎ만</span><span class="reaction-value"><strong data-countup="${data.pureLaughMessages}">${formatNumber(data.pureLaughMessages)}</strong>건 (${pct}%)</span></div></div>`);
    }
...
function renderProvenanceAnalysisBadges(data) {
...
function renderProvenanceFooterPrefix(data) {
    if (!data.provenance)
        return "";
