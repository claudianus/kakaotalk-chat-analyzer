...
function renderParticipantsFold(data) {
    ...
    if (n === 0)
        return "";
    return panel(`참여자 랭킹 · 전체 ${formatNumber(n)}`, "접힌 목록 없이 메시지 수 기준 참여자를 한 번에 보여줍니다.", renderParticipants(data.participants));
}
...
function renderToneSignalsPanel(data) {
    ...
    if (blocks.length === 0) {
        return panel("톤·감정 신호", "비속어 패턴·감정 분석(선택) 요약입니다.", `<p style="margin:0;color:var(--muted);font-size:13px">감지된 비속 패턴이 없거나, 감정 분석 샘플이 부족합니다.</p>`);
    }
