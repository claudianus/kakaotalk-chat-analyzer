...
function renderKeywordSnapshot(items) {
    const note = '<p class="kw-note"><strong>KR-WordRank</strong>로 본문 어절을 뽑고, 해시태그·슬랭을 보조로 더합니다. 막대·표의 숫자는 <strong>해당 단어가 들어간 메시지 수</strong>예요(상대 점수 아님). 위 <a href="#s-viz" data-kca-jump="s-viz">인터랙티브 차트</a>에서 워드클라우드·전체 표를 볼 수 있어요.</p>';
    if (items.length === 0) {
