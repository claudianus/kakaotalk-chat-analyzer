...
const ARCHETYPES = [
    ...
    {
        id: "longform",
        score: (p, _r, avgLen) => p.averageLength >= Math.max(32, avgLen * 1.35) ? 68 + p.averageLength : 0,
    ...
    {
        id: "compact",
        score: (p, _r, avgLen) => p.averageLength > 0 && p.averageLength <= avgLen * 0.72 && p.messages >= 20 ? 55 + p.messages * 0.1 : 0,
...
function pickArchetype(p, rank, avgLen, usedIds) {
...
function distinctiveStatHook(p, rank, used) {
...
function sanitizeLlmLine(value, used) {
    const v = value?.trim();
    if (!v || v.length < 4)
        return null;
    if (isAiSlopText(v))
...
export function resolveCharacterCards(data) {
    ...
    if (top.length === 0)
        return [];
    const avgLen = top.reduce((sum, p) => sum + p.averageLength, 0) / Math.max(top.length, 1);
    ...
    return top.map((p, rank) => {
        ...
        if (!llmTagline)
            usedTaglines.add(tagline.toLowerCase());
