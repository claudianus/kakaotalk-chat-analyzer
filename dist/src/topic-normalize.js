...
const TRAILING_PARTICLE_RE = /(?:이|가|은|는|을|를|에|의|로|와|과|도|만|요|죠|네|지|서|야|다|함|임|음|들)$/u;
export function normalizeTopicTerm(raw) {
    let t = canonicalKeywordToken(raw.trim());
    if (!t)
        return "";
    if (TOPIC_TERM_ALIASES[t])
        return TOPIC_TERM_ALIASES[t];
...
export function normalizeTopicTerms(terms) {
    const seen = new Set();
    const out = [];
...
/** 제목 · 구분 — 순서 무관 동일 키 */
export function topicPairKey(title) {
    const parts = title
        .split(/\s*·\s*/)
        .map((p) => normalizeTopicTerm(p))
        .filter((p) => p.length >= 2);
    if (parts.length === 0)
        return normalizeTopicTerm(title);
...
export function topicSimilarity(a, b) {
    ...
    if (jac >= 0.42)
        return jac;
    if (themeLead(a) === themeLead(b) && jac >= 0.3)
        return Math.max(jac, 0.45);
    if (sharedAnchorSimilarity(setA, setB))
        return Math.max(jac, 0.5);
    return jac;
}
export function normalizedTermsKey(terms) {
    return normalizeTopicTerms(terms)
        .sort()
        .slice(0, 5)
        .join("\t");
}
//# sourceMappingURL=topic-normalize.js.map