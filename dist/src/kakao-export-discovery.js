...
export async function resolveKakaoExport(options = {}) {
    const dir = options.dir ?? defaultKakaoCsvDir();
    const index = options.index ?? 0;
    const minBytes = options.minBytes ?? 1;
    const all = await listKakaoExports(dir);
    if (all.length === 0) {
        throw new Error(`${dir}에 KakaoTalk*.csv가 없습니다. 카카오톡에서 대화보내기 후 CSV를 저장하거나 KCA_CSV_DIR로 폴더를 지정하세요.`);
    }
    if (index < 0 || index >= all.length) {
        throw new Error(`--pick ${index}은(는) 범위를 벗어났습니다. ${dir}에 ${all.length}개의 KakaoTalk CSV가 있습니다 (0=최신).`);
    }
...
export function formatExportPickLine(file, roomName) {
...
//# sourceMappingURL=kakao-export-discovery.js.map