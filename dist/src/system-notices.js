...
const SHOP_SEARCH_INLINE = /^샵검색\s+#?(\S.{1,78})$/u;
...
export function isOpenChatBoilerplate(text) {
    const t = text.trim().replace(/\s+/g, " ");
    if (t.length < 24)
        return false;
    if (OPEN_CHAT_WELCOME_RE.test(t))
        return true;
...
export function detectSystemNoticeLine(line) {
    ...
    if (MANAGER_LINE.test(t))
        return "manager";
    if (SHOP_SEARCH_LINE.test(t) || SHOP_SEARCH_INLINE.test(t) || /^샵검색\s+\S/u.test(t))
        return "shopSearch";
...
function normalizeShopSearchTag(raw) {
...
export function extractShopSearchTag(line) {
    const t = normalizeNoticeLine(line);
    const colon = t.match(SHOP_SEARCH_LINE);
    if (colon?.[1])
        return normalizeShopSearchTag(colon[1]);
    const inline = t.match(SHOP_SEARCH_INLINE);
    if (inline?.[1])
        return normalizeShopSearchTag(inline[1]);
    if (SHOP_SEARCH_HASH_ONLY.test(t))
        return normalizeShopSearchTag(t);
    if (/^샵검색\s+/u.test(t)) {
        const tail = t.replace(/^샵검색\s+/u, "").trim();
        if (tail.length >= 1)
            return normalizeShopSearchTag(tail);
    }
    return null;
}
