...
export function renderLlmEpisodeStrip(data) {
    ...
    if (!cards?.length)
        return "";
    const inner = cards
        .map((c) => `<article class="llm-episode-card" role="listitem" data-observe>
...
export function renderLlmMomentsBlock(data) {
    ...
    const paras = data.narrative.paragraphs
        .map((p) => `<article class="narrative-quote-card" data-observe><p>${renderHighlightLine(p)}</p></article>`)
        .join("");
    const momentCards = (moments ?? [])
        .map((m) => `<article class="llm-moment-card" role="listitem" data-observe>
...
export function renderLlmRelationshipBeats(data) {
    ...
    if (!beats?.length)
        return "";
    const rows = beats
        .map((b) => `<li class="llm-beat-card" data-observe><strong>${escapeHtml(b.pair)}</strong>${b.role ? ` <em>${escapeHtml(b.role)}</em>` : ""}<span>${renderHighlightLine(b.beat)}</span></li>`)
        .join("");
...
export function renderLlmCharacterCards(data) {
    ...
    if (cards.length === 0)
        return "";
    const inner = cards
        .map((c) => `<article class="llm-char-card" role="listitem" data-observe>
...
export function renderLlmInsideJokes(data) {
    ...
    if (!jokes?.length)
        return "";
    const chips = jokes
        .map((j) => `<span class="llm-meme-chip" title="${escapeHtml(j.whyFunny)}" data-observe>${renderHighlightLine(j.label)}</span>`)
        .join("");
...
export function renderLlmEraLabels(data) {
    ...
    if (!eras?.length)
        return "";
    const rows = eras
        .map((e) => `<li data-observe><strong>${escapeHtml(e.label)}</strong><span>${renderHighlightLine(e.detail)}</span></li>`)
        .join("");
...
export function renderLlmDayMicroStories(data) {
    ...
    const rows = days
        .slice(0, 4)
        .map((d) => `<li data-observe><time datetime="${escapeHtml(d.date)}">${escapeHtml(d.date)}</time> ${renderHighlightLine(d.line)}</li>`)
        .join("");
...
function renderMiniHourlyBar(hourly, highlightHour) {
    ...
    const bars = hourly
        ...
        .join("");
    const peakLabel = highlightHour !== null ? `가장 붐빈 ${highlightHour}시` : "시간대별 메시지 수";
...
function mapSentimentWeather(s, energy) {
    ...
    if (score > 0.05)
        return { icon: "🌤️", cls: "partly-sunny", label: "대체로 맑음" };
    if (score > -0.05)
        return { icon: "⛅", cls: "cloud", label: "구름" };
    if (score > -0.3)
        return { icon: "🌥️", cls: "overcast", label: "흐림" };
    return { icon: "🌧️", cls: "rain", label: "비" };
}
/** 최근 7일 감정 흐름을 날씨 아이콘으로 렌더링 */
export function renderSentimentWeatherStrip(data) {
    const sourceDays = data.recentSnapshot && data.recentSnapshot.week.length >= 3
...
export function renderRoomCultureStrip(data) {
    ...
    if (repeated.length === 0 && jokes.length === 0)
        return "";
    function formatCultureLabel(label) {
        ...
        const withLinks = renderHighlightLine(label).replace(/(https?:\/\/[^\s<]+)/g, (url) => `<a class="culture-link" href="${url}" target="_blank" rel="noopener noreferrer">${url}</a>`);
        return withLinks;
    }
    const phraseCards = repeated
        .map((r) => `<article class="culture-card culture-card--phrase" data-observe role="listitem">
    ...
    const jokeCards = jokes
        .map((j) => `<article class="culture-card culture-card--joke" data-observe role="listitem" title="${escapeHtml(j.why)}">
          <span class="culture-badge">밈</span>
          <span class="culture-label">${formatCultureLabel(j.label)}</span>
        </article>`)
        .join("");
    const laughBadge = data.pureLaughMessages > 0
