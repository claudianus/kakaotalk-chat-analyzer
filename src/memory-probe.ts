...
function probeMode(): MemoryProbeMode {
...
export function parseDarwinVmStatPages(vmStatText: string): {
  free: number;
  inactive: number;
  speculative: number;
} {
  ...
  for (const line of vmStatText.split("\n")) {
    ...
    if (m[1] === "free") free = n;
    else if (m[1] === "inactive") inactive = n;
    else if (m[1] === "speculative") speculative = n;
  }
  return { free, inactive, speculative };
}

export function darwinAvailableBytesFromVmStat(
  vmStatText: string,
  pageSize: number,
): number {
...
function darwinPageSize(): number {
  try {
...
function probeDarwinAvailableBytes(): number | null {
  try {
...
export function parseLinuxMemAvailableKb(meminfoText: string): number | null {
  for (const line of meminfoText.split("\n")) {
    if (line.startsWith("MemAvailable:")) {
      const kb = Number.parseInt(line.split(/\s+/)[1] ?? "", 10);
      if (Number.isFinite(kb) && kb > 0) return kb;
    }
  }
  for (const line of meminfoText.split("\n")) {
...
function probeLinuxAvailableBytes(): number | null {
  try {
...
function probeFallbackAvailableBytes(): number {
  const free = freemem();
...
export function probeAvailableMemoryBytes(): number {
  if (probeMode() === "free") return freemem();

  const plat = platform();
  let specific: number | null = null;
  if (plat === "darwin") specific = probeDarwinAvailableBytes();
  else if (plat === "linux") specific = probeLinuxAvailableBytes();

  if (specific !== null && specific > 0) return specific;
  return probeFallbackAvailableBytes();
}

export function probeFreeMemoryBytes(): number {
  return freemem();
}
