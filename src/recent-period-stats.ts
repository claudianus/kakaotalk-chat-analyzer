...
function top3SharePercent(counts: number[], total: number): number {
  if (total <= 0 || counts.length === 0) return 0;
  const top3 = [...counts].sort((a, b) => b - a).slice(0, 3).reduce((s, c) => s + c, 0);
  return round1((top3 / total) * 100);
}

function weekendShareForDates(daily: Map<string, number>, dates: string[]): number {
  let weekend = 0;
  let total = 0;
...
function nightShareForDates(dailyHourly: Map<string, number[]>, dates: string[]): number {
  let night = 0;
  let total = 0;
  for (const date of dates) {
...
export function buildRecentPeriodInsights(input: BuildRecentPeriodInsightsInput): RecentPeriodInsights {
  ...
  const fmtGini = (n: number | null) => (n === null ? "—" : String(n));
