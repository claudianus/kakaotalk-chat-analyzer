...
export function isDiscourseTerm(term: string): boolean {
  const w = term.trim();
  if (!w) return true;
  if (DISCOURSE_LEXICON.has(w)) return true;
...
export function discourseRatio(terms: readonly string[]): number {
  if (terms.length === 0) return 1;
  return terms.filter((t) => isDiscourseTerm(t)).length / terms.length;
}

export function mergeDiscourseIntoStopwords(base: Set<string>): Set<string> {
  for (const w of DISCOURSE_LEXICON) base.add(w);
  return base;
}
