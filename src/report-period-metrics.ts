...
export function renderPeriodMetricsComparison(data: ReportData): string {
  const rp = data.recentPeriodInsights;
  if (!rp || rp.metrics.length === 0) return "";

  const rows = rp.metrics
    .map(
      (m) =>
        `<tr><td>${escapeHtml(m.label)}</td><td class="num">${escapeHtml(m.whole)}</td><td class="num period-metrics-week">${escapeHtml(m.week)}</td></tr>`,
    )
    .join("");
...
export function renderWeekTopSendersStrip(data: ReportData): string {
  ...
  const items = senders
    .map(
      (s, i) =>
        `<li class="week-sender-item" data-observe><span class="week-sender-rank">${i + 1}</span><span class="week-sender-name">${escapeHtml(s.alias)}</span><span class="week-sender-count">${s.count}건 · ${s.sharePercent}%</span></li>`,
    )
    .join("");
