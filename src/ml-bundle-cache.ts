...
export function mlModelsReleaseTag(): string {
...
function pinnedReleaseAssetUrl(assetName: string): string {
...
interface LazyOnnxZipInstall {
  label: string;
  assetName: string;
  extractDirName: string;
  resolveSrc: (tmpExtract: string) => string | undefined;
  destModelId: string;
  isReady: () => boolean;
}

async function installLazyOnnxZip(opts: LazyOnnxZipInstall): Promise<boolean> {
  ...
  try {
    ...
    if (!fetched) {
      process.stderr.write(
        `[kca] ${opts.label} Release zip(${opts.assetName})을 찾지 못했습니다. ` +
