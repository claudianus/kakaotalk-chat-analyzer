...
function defaultTagline(p: ParticipantStat, rank: number): string {
  if (rank === 0) return "이 방에서 가장 많이 말한 사람";
  if (p.nightMessages / Math.max(p.messages, 1) >= 0.3) return "밤늦게까지 채팅창을 지키는 타입";
  if (p.averageLength >= 35) return "길게 맥락을 설명하는 편";
  if (p.linkMessages >= 8) return "링크와 자료를 자주 던지는 편";
  if (p.attachmentMessages >= 8) return "사진·첨부로 분위기를 만드는 편";
  if (p.maxConsecutive >= 10) return "한 번 붙으면 길게 이어 말하는 편";
