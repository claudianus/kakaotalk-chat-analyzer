...
function loadModelBuffers(): ModelFiles {
...
async function downloadModelArchive(destTgz: string): Promise<void> {
...
function extractModelArchive(tgzPath: string): void {
...
async function ensureModelOnDisk(): Promise<void> {
...
export function getKiwiRuntime(): Kiwi | null {
  return readyKiwi;
}

export function isKiwiReady(): boolean {
...
export function kiwiKeywordTokens(text: string): string[] {
  ...
  for (const t of tokens) {
    if (!isKeywordPos(t.tag)) continue;
...
function isKeywordPos(tag: string): boolean {
  if (tag === "SL" || tag === "SN") return true;
  if (tag.startsWith("NN")) return true;
  if (tag === "NR") return true;
  if (tag === "W_URL" || tag === "W_EMAIL") return true;
  if (tag === "W_HASHTAG") return true;
  return false;
}

function normalizeKiwiForm(raw: string): string {
  let w = raw.trim();
...
/** 캐시 디렉터리(테스트·진단용) */
export function kiwiCacheDir(): string {
  return CACHE_ROOT;
}

export function resetKiwiRuntimeForTests(): void {
  readyKiwi = null;
  initPromise = null;
  initFailed = false;
  pendingUserWords = [];
