...
function focusDayStats(day: DailySnapshot | null | undefined): {
  messages: number;
  participants: number;
  lead: string;
  keywords: string;
...
function renderFocusColumn(title: string, badge: string, stats: ReturnType<typeof focusDayStats>, extra?: string): string {
...
export function renderRecentFocusDeck(data: ReportData): string {
  const snap = data.recentSnapshot;
  if (!snap) return "";

  const today = focusDayStats(snap.today);
  const week: RecentSnapshot = snap;
  const weekStats = {
  ...
  const todayPeak =
    snap.today?.peakHour !== null && snap.today?.peakHour !== undefined
