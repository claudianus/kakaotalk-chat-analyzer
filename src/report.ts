...
<body class="kca-oled">
  ...
  <main>
    ...
    <div class="kca-report-flow">
      ...
      ${
        hasCalendarHeatmap(data)
          ...
          : panel(
              "일별 활동 (CSS)",
              "칸 색 = 메시지 밀도.",
              renderDaily(data.daily, data.burstDays),
              { id: "s-charts" },
            )
      }
      ${renderParticipantsFold(data, hasCalendarHeatmap(data) ? { id: "s-charts" } : undefined)}
      ${panel(
        `글자 수 랭킹 · 상위 ${formatNumber(Math.min(data.participantsByCharacters.length, 10))}`,
        "건수가 아닌 글자 수 기준 랭킹.",
        renderParticipantsByCharacters(data.participantsByCharacters),
      )}
      ...
      ${panel(
        "카카오톡 시스템·운영 알림",
        "입·퇴장, 삭제·가림, 강퇴 등 시스템 문구 집계.",
        renderRoomEvents(data.roomEvents, data.summary.totalMessages, data.roomPulse),
      )}
...
function renderTopicTrendSection(data: ReportData): string {
  const trend = data.smartTopicTrend;
  if (!trend || trend.items.length < 2) return "";
  const periodHeader =
    trend.granularity === "daily" ? "날짜" : trend.granularity === "weekly" ? "주차" : "월";
...
function renderParticipantsFold(data: ReportData, opts?: { id?: string }): string {
...
function renderWeeklySparklineSection(): string {
...
function renderInsightDeck(data: ReportData): string {
  const ins = data.insights;
  const sessionMetric = insMetric("💬 대화 세션",
    ins.avgMessagesPerSession === null
      ? `${ins.sessionCount}개`
...
function honorificBarPercents(p: NonNullable<ReportData["honorificInsight"]>["participants"][number]): {
  formal: number;
  casual: number;
  neutral: number;
...
function renderHonorificInsight(honorific: ReportData["honorificInsight"]): string {
  ...
  const rows = honorific.participants
    ...
    .map((p) => {
      ...
      const rowCls =
        p.dominantStyle === "insufficient" ? "honorific-bar-row honorific-bar-row--insufficient" : "honorific-bar-row";
      const trackInner =
        p.dominantStyle === "insufficient"
...
function renderHeroRhythmCard(data: ReportData): string {
  const ins = data.insights;
  const pace = data.conversationPace;
...
function renderParticipantsByCharacters(participants: ParticipantStat[]): string {
  if (participants.length === 0) {
  ...
  const rows = participants
    .map(
      (p) =>
        `<tr><td>${escapeHtml(p.alias)}</td><td class="num">${formatNumber(p.characters)}</td><td class="num">${p.characterSharePercent}%</td><td class="num">${formatNumber(p.messages)}</td><td class="num">${p.averageLength}</td></tr>`,
    )
    .join("");
...
function renderProfanityInline(
  profanity: ReportData["profanity"],
...
function renderOpenChatInsightCard(data: ReportData): string {
  const profile = openChatProfileFromReport(data);
  if (!profile.likely) return "";
  const ev = data.roomEvents;
  const boiler =
    data.openChatBoilerplateExcluded > 0
...
function renderShopSearchPromoted(data: ReportData): string {
  const profile = openChatProfileFromReport(data);
  if (!profile.likely) return "";
  return renderShopSearchSection(data, true);
}

function renderShopSearchSection(data: ReportData, promoted = false): string {
  const topics = data.shopSearchTopics;
  const ev = data.roomEvents;
  const noticeCount = ev.shopSearchCount;
  if (topics.length === 0 && noticeCount === 0) return "";
  if (promoted === false && openChatProfileFromReport(data).likely) return "";
  if (topics.length === 0) {
    return panel(
      "샵검색 키워드",
  ...
  return panel(
    ...
    renderCountBars(topics) + footnote + debug,
    { id: "s-shopsearch" },
  );
}
...
function renderCountBars(items: CountItem[]): string {
  ...
  return `<div class="bars" data-observe>${items
    .map((item) => {
      ...
      const laneTip =
        item.keywordLane === "bm25" && item.distinctiveRank
