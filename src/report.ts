...
function renderParticipantsByCharacters(participants: ParticipantStat[]): string {
  ...
  const cards = participants
    .map(
      (p) =>
        `<li class="rank-card"><strong>${escapeHtml(p.alias)}</strong><span class="rank-card-stat">${formatNumber(p.characters)}자 · ${p.characterSharePercent}%</span><span class="rank-card-meta">메시지 ${formatNumber(p.messages)}건 · 평균 ${p.averageLength}자</span></li>`,
    )
    .join("");
...
function renderShopSearchSection(data: ReportData, promoted = false): string {
  ...
  return `<section id="s-shopsearch" style="margin-bottom:14px">${panel(
    title,
