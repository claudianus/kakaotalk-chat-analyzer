...
export interface ChartPayload {
  ...
  smartTopicTrend: {
    ...
    label: string;
    hint: string;
    items: { period: string; topics: { name: string; value: number }[] }[];
  } | null;
}
...
function chartDensityClass(data: ReportData): string {
  const chartSignals = [
  ...
  if (chartSignals >= 6) return "viz-grid--dense";
  if (chartSignals >= 4) return "viz-grid--balanced";
