...
async function importTransformers(): Promise<typeof import("@huggingface/transformers")> {
  try {
    ...
    throw new Error(
      "[kca] 독성 ML은 optional dependency @huggingface/transformers 가 필요합니다. KCA_NO_TOXICITY=1 로 끄세요.",
    );
  }
}
...
async function loadPipeline(requestedModelId: string): Promise<ClassificationPipeline> {
  ...
  if (!pipelinePromise) {
    pipelinePromise = withQuietMlStderr(async () => {
      ...
      const { pipeline } = mod;
      const load = () =>
        pipeline("text-classification", modelId, { dtype }) as Promise<ClassificationPipeline>;
      let pipe: ClassificationPipeline;
      if (isLocalBundledToxicityModel(modelId)) {
        try {
          ...
          if (isTransformersFetchError(err)) {
            process.stderr.write(
              `[kca] 독성 번들 로드 실패(${modelId}) → Hub 폰백(${HUB_KCELECTRA_TOXICITY})\n`,
            );
            const hubLoad = () =>
              pipeline("text-classification", HUB_KCELECTRA_TOXICITY, { dtype }) as Promise<ClassificationPipeline>;
