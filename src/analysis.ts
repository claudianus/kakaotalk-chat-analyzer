...
export function buildReportData(result: ParseResult, options?: { privacy?: PrivacyMode; top?: number }): ReportData {
  ...
  for (const record of result.records) {
    ...
    if (prevMs !== null) {
      ...
      if (delta > 0 && delta <= MAX_GAP_MS) gapsMs.push(delta);
    }
    prevMs = ms;
  ...
  const medianReplyGapMinutes =
    gapsMs.length > 0 ? round(medianSorted([...gapsMs].sort((a, b) => a - b)) / 60_000, 1) : null;
...
export function parseChatRoomNameFromExportPath(filePath: string): string {
  ...
  if (room.length > 0) return room.length > 120 ? `${room.slice(0, 117)}...` : room;
...
function buildSenderLabels(records: ChatRecord[], privacy: PrivacyMode): Map<string, string> {
  const unique: string[] = [];
...
export function maskPartialDisplayName(raw: string): string {
  const s = raw.trim();
  if (!s) return "?";
  const chars = [...s];
