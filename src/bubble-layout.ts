...
export function resolveBubbleOverlaps(items: BubbleLayoutInput[]): BubbleLayoutResult[] {
  const sorted = items
    .map((item, index) => ({ item, index }))
    .sort((a, b) => b.item.scale - a.item.scale || a.index - b.index);

  const placed: BubbleLayoutResult[] = [];
  ...
  const out = new Array<BubbleLayoutResult>(items.length);
