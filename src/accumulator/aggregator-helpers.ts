...
type RoleCandidate = { role: string; confidence: number; reason: string; score: number };
...
function pickParticipantRole(
  ...
  const candidates = collectRoleCandidates(
    p,
    rank,
    laughRate,
    laughCount,
...
function fallbackParticipantRole(p: ParticipantStat, rank: number, usedRoles: Set<string>): RoleCandidate {
  const variants: RoleCandidate[] = [
    {
      role: "말 많은 1위",
...
export function buildParticipantRoles(
  ...
  const evalOne = (p: ParticipantStat, rank: number, force: boolean): void => {
    if (used.has(p.alias)) return;
    ...
    const picked =
      pickParticipantRole(
        ...
        shortCount,
        p.nightMessages / msg,
        p.attachmentMessages / msg,
        p.linkMessages / msg,
        avgLengthOverall,
        usedRoles,
      ) ?? (force ? fallbackParticipantRole(p, rank, usedRoles) : null);
    if (!picked) return;
    if (!force && picked.confidence < PARTICIPANT_ROLE_MIN_CONFIDENCE) return;
...
export function buildDaySnapshotHeadline(day: {
  ...
  if (day.messageCount === 0) return "이 날은 대화가 없었어요.";
  const chunks: string[] = [];
  if (day.vsAvg >= 1.8) chunks.push(`평소의 ${day.vsAvg}배로 붐빔`);
  else if (day.vsAvg <= 0.45) chunks.push("조용한 날");
  const lead = day.topSenders[0];
  if (lead) chunks.push(`${lead.alias} ${lead.count}건 주도`);
  if (day.keywords.length) chunks.push(day.keywords.slice(0, 2).join("·"));
  if (day.peakHour !== null) chunks.push(`${day.peakHour}시에 가장 붐빔`);
