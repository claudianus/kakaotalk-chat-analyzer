...
import {
  ...
  HUB_KOREAN_KURE_V1,
  SEMANTIC_HEADROOM_BGE_GB,
  SEMANTIC_HEADROOM_KURE_BALANCED_GB,
  SEMANTIC_HEADROOM_KURE_GB,
} from "./ml/model-ids.js";
import {
...
export function resolveDefaultSemanticHubId(
  ...
  if (preset === "quality" || preset === "ultra") return HUB_GRANITE_EMBED_97M;
...
export function shouldPreferBundledKure(
  preset: AnalysisPresetName,
  headroomGb: number,
): boolean {
  if (process.env.KCA_LEGACY_MODELS === "1") {
    if (process.env.KCA_PREFER_BUNDLED_SEMANTIC === "0") return false;
    if (headroomGb < SEMANTIC_HEADROOM_KURE_GB) return false;
...
export function resolveBundledSemanticModelId(
  preset: AnalysisPresetName,
  headroomGb: number,
): string {
...
export function shouldPreferBundledSemanticPolicy(
  preset: AnalysisPresetName,
  headroomGb: number,
): boolean {
  if (process.env.KCA_PREFER_BUNDLED_SEMANTIC === "0") return false;
  if (preset === "ultra" || preset === "quality") return true;
  if (preset === "balanced") return headroomGb < SEMANTIC_HEADROOM_KURE_BALANCED_GB;
  return true;
}

/** 오프라인 번들 우선 — `KCA_PREFER_BUNDLED_SEMANTIC=0` 이면 Hub */
export function shouldPreferBundledSemantic(
  preset: AnalysisPresetName,
  headroomGb: number,
): boolean {
  if (isBundledGraniteEmbedModelReady()) return true;
  if (!isBundledEmbedModelReady()) return false;
  return shouldPreferBundledSemanticPolicy(preset, headroomGb);
}

/** 로드 실패 시 순차 폰백 — Granite → KoELECTRA → 번들 */
export function semanticEmbeddingFallbackIds(primary: string): string[] {
  ...
  // 기본 KoELECTRA 계열
    ...
    if (id.includes("kure")) tiers.unshift(HUB_KOREAN_KURE_V1);
    if (id.includes("bge")) tiers.unshift(HUB_BGE_M3);
