...
export function isNoiseKeyword(label: string): boolean {
  const w = label.trim();
  if (w.length < 2) return true;
  if (/^\d{1,4}$/.test(w)) return true;
  if (/^[a-z]{1,3}$/i.test(w) && w.length <= 3) return true;
  if (/^[a-z]+$/i.test(w) && w.length <= 8 && NOISE_LATIN.has(w.toLowerCase())) return true;
  ...
  if (!w.includes(" ") && w.length <= 3 && VERB_FRAGMENT_RE.test(w)) return true;
