...
export class ReportAggregator {
  ...
  private readonly gapsMs: number[] = [];
  ...
  consume(record: ChatRecord): void {
    ...
    if (this.prevMs !== null) {
      ...
      if (delta > 0 && delta <= MAX_GAP_MS) this.gapsMs.push(delta);
    }
    this.prevMs = ms;
  ...
  finalize(meta: FinalizeSourceMeta): ReportData {
    ...
    const medianReplyGapMinutes =
      this.gapsMs.length > 0 ? round(medianSorted([...this.gapsMs].sort((a, b) => a - b)) / 60_000, 1) : null;
    ...
    const burstGapUnder1mPercent =
      this.gapsMs.length > 0 ? round((burstUnder1m / this.gapsMs.length) * 100, 1) : null;
    const gapOver60mPercent =
      this.gapsMs.length > 0 ? round((gapOver60m / this.gapsMs.length) * 100, 1) : null;
