...
import {
  analyzeSentimentFromSamples,
  preloadSentimentPipeline,
  type SentimentBatchItem,
} from "./sentiment-analyze.js";
import { preloadSemanticPipeline } from "./semantic-keywords.js";
import { analyzeToxicityFromSamples, preloadToxicityPipeline } from "./toxicity-analyze.js";
...
export async function preloadUtteranceMl(
  opts: UtteranceMlPassOptions,
): Promise<void> {
...
export async function runUtteranceMlPass(
  agg: ReportAggregator,
  opts: UtteranceMlPassOptions,
): Promise<UtteranceMlPassResult> {
  ...
  const sentimentSamples =
    opts.useSentiment || opts.useToxicity ? agg.drainSentimentSamples() : [];
  ...
  if (sentimentSamples.length >= 48) {
    ...
    if (opts.useSentiment) {
      if (opts.showProgress) logReportProgress({ phase: "감정 분석", current: 0 });
      try {
        const stats = await analyzeSentimentFromSamples(
          ...
          opts.showProgress
            ? (current, total) =>
                logReportProgress({ phase: "감정 분석", current, total })
            : undefined,
          opts.buildOptions,
        );
        if (stats) {
  ...
  if (opts.useSemantic) {
    ...
    if (samples.length >= 48) {
      ...
      if (opts.showProgress) logReportProgress({ phase: "시맨틱 키워드", current: 0 });
      try {
        const items = await extractSemanticKeywords(samples, {
          ...
          onProgress: opts.showProgress
            ? (current, total) =>
                logReportProgress({ phase: "시맨틱 키워드", current, total })
            : undefined,
        });
        if (items.length > 0) {
