...
export function keywordLaneCaps(messageCount: number, outputLimit: number): {
  freqCap: number;
  bm25Cap: number;
  outputLimit: number;
} {
  ...
  const freqCap =
    n >= 50_000 ? 80 : n >= 10_000 ? 60 : n >= 2_000 ? 48 : 32;
  const bm25Cap = Math.min(200, Math.floor(80 + Math.sqrt(n)));
  return { freqCap, bm25Cap, outputLimit };
}

function normLogHits(hits: number, maxHits: number): number {
...
function toCountItem(
  ...
  const lane: CountItem["keywordLane"] =
    m.freqRank && m.bm25Rank ? "both" : m.bm25Rank ? "bm25" : m.freqRank ? "freq" : undefined;
  return {
    label,
...
export function mergeDualLaneKeywords(
  ...
  const upsert = (item: KeywordRankItem, laneRrf: number) => {
  ...
  suppTop.forEach((item, i) => {
    ...
    if (corpusHits === undefined) return;
    const prev = meta.get(item.label);
    if (prev) prev.rrf += rrf(i + 1) * semanticSupplementWeight;
    else {
      meta.set(item.label, {
